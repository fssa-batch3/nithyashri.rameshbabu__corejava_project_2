...
public class UserValidator {
	// For example, they ensure the email looks like an email, the password is
	// strong, and the name isn't too short. If these rules aren't followed, they
	// raise their hands and say "Hold on, something's not right via exception."
	...
	public static boolean validateUser(UserModel user) throws InvalidUserException {
		...
		if (validateEmail(user.getEmail()) && validatePassword(user.getPassword()) && validateName(user.getName())) {
			return true;
		} else {
